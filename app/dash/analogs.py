...
def get_analogs():
    with engine.connect() as conn:
        ...
        df.columns = ["ID", "Product Name 1", "Product Name 2"]
        return df
...
def update_analog_table(n_clicks, rows, columns):
    ...
    with engine.begin() as conn:
        str_list = [
            f"({row['ID']}, (SELECT id FROM product WHERE name='{row['Product Name 1']}'), (SELECT id FROM product WHERE name='{row['Product Name 2']}'))"
